...
while Entry_Date + timedelta(days = k) != Exit_Date:
	...
	fig.add_trace(go.Scatter(x=Date_Divider_DF["Index_Time"], y = Date_Divider_DF["Index_Value"], name = "Index Test", yaxis="y2", mode='markers', line=dict(color='#bab6b6'), line_width=0.7, showlegend = False))
...
for i in range((Sell_Dist)[0], (Sell_Dist)[1]+1, 1):
	...
	fig.add_trace(go.Scatter(x=Final_DF.index, y=Final_DF["Change"+str(i)], mode = 'markers', legendgrouptitle_text = (str(int(i/5)) + "Group"), legendgroup= int(i/5), customdata = Final_DF["FINAL"], name = str(i).rjust(4), hovertemplate='Profit: (%{y:5d} )   |   %{customdata}'))#, visible='legendonly'))
	
fig.add_trace(go.Scatter(x= Index_csv_2.index, y= Index_csv_2["o"], yaxis="y2", mode='markers', name = Index_Name, line=dict(color='blue'), line_width=0.8, legendrank = 1))
